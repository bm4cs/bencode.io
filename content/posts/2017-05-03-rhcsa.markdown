...
- Partitions: the classical method of allocating dedicated storage to types of data
- LVM (Logical Volume Manager) volumes: a higher level abstraction of storage, with rich features; resizing, snapshots
- Stratis: RHEL8+ nextgen volume manager, implemented in user space, to better support cloud and containerised environments
- Virtual Data Optimiser (VDO): focuses on storage data in the most efficient way, by introducing concepts such as deduplicated and compressed storage pools
...
Good old `fdisk`. Sanely defaults when working with MBR partitions, e.g. if 3 primary partitions exist, it will default the next partition to type extended, which you can then fill up with logical partitions.
