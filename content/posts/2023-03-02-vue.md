...
The magic of the Composition API acsends to god mode, with compile-time syntactic sugar, and is the recommended approach for SFC that are purely based on the Composition API. It involves including a `setup` attribute to the `<script>` block like so:
