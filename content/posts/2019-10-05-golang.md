...
- Unlike arrays are reference based (i.e. use pointers to the underlying array) and can be extended
...
- Internally a slices have a header that defines:
  - The current length (as shown by `len()`)
  - The remaining capacity in the underlying array (as shown by `cap()`)
