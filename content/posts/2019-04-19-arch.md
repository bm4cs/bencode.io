...
* Fonts: `ttf-fira-code`, `ttf-font-awesome`, `noto-fonts-cjk`, `noto-fonts-emoji`, `noto-fonts`, `ttf-freefont`, `ttf-ms-fonts`, `ttf-linux-libertine`, `ttf-dejavu`, `ttf-inconsolata`, `ttf-ubuntu-font-family`
* Font management: `xorg-xfontsel` (gives dropdown for selecting parts of a font name and previews the font selected), `xorg-xfontsel` (list fonts by name, and can show how wildcards and aliases will be interpreted by the XLFD system)
* File manager: `nnn`
...
Apply [patches](https://dwm.suckless.org/patches/) for specific features wanted.
...
This is a worth while endevour to take. I now have a very tailored system that works exactly how I want, without any bloat, and its FAST. Finally if interested, all my configuration from `vim` to `st` and `dwm` is in my [dotfiles](https://github.com/bm4cs/dotfiles) git repo.
