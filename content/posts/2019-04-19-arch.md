...
lastmod: "2022-11-16 19:49:38+11:00"
comments: false
categories:
  - linux
tags:
...
- [File is corrupt invalid or corrupted package - PGP signature](#file-is-corrupt-invalid-or-corrupted-package---pgp-signature)
...
mkswap ext4 /dev/mapper/vg-swap
```
...
So mounts are automatically applied as part of the system boot process, add them to `/etc/fstab` as UUID's, which are more robust than device names which can change.
...
Finally setup crypttab (if decide to encrypt volume), a passphrase of `none` will cause an interactive prompt at boot:

```
nvim /etc/crypttab
luks    /dev/nvme0n1p3    none
```
...
## File is corrupt invalid or corrupted package - PGP signature
