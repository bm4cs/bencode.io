...
- [Senzing V4 SDK Setup on Metal](#senzing-v4-sdk-setup-on-metal)
  - [Native Senzing SDK Setup](#native-senzing-sdk-setup)
  - [.NET SDK Setup](#net-sdk-setup)
    - [Setup Local NuGet Source](#setup-local-nuget-source)
  - [Senzing V4 C# Snippets](#senzing-v4-c-snippets)
- [Senzing V4 CLI Tools](#senzing-v4-cli-tools)
  ...
  - [sz\_explorer](#sz_explorer)
    - [get](#get)
    - [how (tree)](#how-tree)
- [Senzing Weirdness](#senzing-weirdness)
  - [Typed models vs loose JSON strings](#typed-models-vs-loose-json-strings)
  - [TODO](#todo)
  - [Info Messages aka SZ\_WITH\_INFO](#info-messages-aka-sz_with_info)
- [Senzing Best practices](#senzing-best-practices)
- [Resources](#resources)
...
Specifically I'm interested in Senzing aka `sz` or `G2`, `G2` being the OG code name of what Senzing is today, which began as a 2009 IBM skunkworks project.
...
The native SDK underpins all other SDKs (e.g. Python, Java, .NET).
...
5. Provision a Senzing environment. This will register the various env vars, python packages, etc so tools like `sz_configtool` (python) will work. First `/opt/senzing/er/bin/sz_create_project ~/senzing && cd ~/senzing`, then `source setupEnv`.
...
Run/debug the snippets with `dotnet run`.

First export connection string:
...
End to end example (i.e. teardown, load, search):

```sh
cd snippets
dotnet run --project initialization/PurgeRepository
dotnet run --project loading/LoadTruthSetWithInfoViaLoop
dotnet run --project searching/SearchRecords
```
...
### Typed models vs loose JSON strings

Senzing is clearly an old C based system. Traits include heavy use of passing unstructured JSON strings arounds, no enum definitions for types such as legal values of `NAME_TYPE`, the .NET SDK is just a thin interop layer over the C-based native libraries, etc.

### TODO

Stewardship
Explainability
Custom configuration (modelling custom attributes)
Reprocessing after configuration changes
Modelling disclosed relationships
Redo records
Dynamic resolution
Elasticsearch integration

### Info Messages aka SZ_WITH_INFO
...
The `AFFECTED_ENTITIES` object contains a list of all entity IDs affected. Separate processes can query the affected entities and synchronize changes and information to downstream systems.

## Senzing Best practices

- Include as much historic context in records as possible
-

## Resources

- [Senzing - How does an Entity ID behave](https://senzing.zendesk.com/hc/en-us/articles/4415858978067-How-does-an-Entity-ID-behave)
- [Senzing - Entity Specification](https://senzing.com/docs/entity_specification/)
- [Senzing - Advanced Mapping Concepts](https://senzing.zendesk.com/hc/en-us/articles/360054097153-Advanced-mapping-concepts)
