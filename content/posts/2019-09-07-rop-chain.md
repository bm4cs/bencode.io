...
Further down, the script even templates out the instruction into a few mainstream languages (ruby, C, python), here's the python:
...
On the remote host destined to catch the remote shell when the exploit triggers (defined by `LHOST` by msfvenom), make sure a server is running to catch the shell when it comes in over TCP/IP. Keeping life simple with netcat:
...
To help shed some light on the sitution, we need to enact a debugger. Given we are on Windows, lets go with Immunity Debugger. Running winamp under Immunity, place a break point on the very first ROP chain gadget (`0x7deb1da7`).
...
* On the *CPU - main thread, module* pane, select the top left hand sub-pane, and locate instruction `0x7deb1da7` by using ctrl+g. Press F2 to drop a break.
* Now trigger the overflow in the Help | Version History menu.
* Immunity will hit the breakpoint.
...
The `ESP` oddly does not line up with the first instruction of the ROP chain, it is 14 bytes ahead of the first ROP gadget instruction. This means as soon as the `RET` of the first gadget is hit, the EIP will jump to the fifth gadget of the ROP chain, not the second gadget as planned! This is not good at all, as 3 important ROP links in the chain are being completely jumped over (i.e. by going from step 1 to step 5 are missing steps 2, 3 and 4). We could modify ESP in immunity directly to test this out, or by throwing some NOP padding so that the ESP lines up to the second piece of the ROP chain.
