...
I worked through the kernel sections in the above, including qualcomm PCIe wifi card, and integrated realtek 8169 ethernet adapter.

Finally I worked through the `lsmod` and `lspci` dumps I made from the LiveCD environment.
...
Due to the GPU crisis of 2021 (its basically impossible to purchase a modern GPU right now, due to the crypto miners) am running my 2016 Radeon RX480.

This runs the Arctic Islands chipset, specifically POLARIS10. The following blobs need to be registered into the custom kernel firmware loader, under generic options in `make menuconfig`:
