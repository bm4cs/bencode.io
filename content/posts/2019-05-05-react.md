...
Effect here means side-effect. We have our render loop, and then other work we want to happen.

`useEffect` allows you to say do a render of this component first so the user can see something, then as soon as the render is done, then do something else. Here we want the user to see our UI first then we want to make a request to the API so we can initialize a list of pizzas.
