...
The `useRef` hook allows you to reference a value that is not directly involved in the component's rendering process. It persists this value across re-renders without triggering a re-render itself when the value changes.

```jsx
import React, { useRef } from "react";

function MyComponent() {
  const inputRef = useRef(null); // Initialise with null
...
Here `inputRef` holds a reference to the `<input>` DOM element. When the button is clicked, `focusInput` is called, and `inputRef.current.focus()` directly interacts with the DOM to set focus on the input field.

Key traits:

- **Persists values across renders**: useRef returns a mutable ref object whose `.current` property can hold any value. This value remains the same across re-renders of the component, unlike variables declared with let or const inside the component, which are re-initialized on each render.
- **Does not trigger re-renders**: Changes to the `.current` property of a ref do not cause the component to re-render. This is a key distinction from useState, which specifically triggers re-renders when its state changes.
- **Direct DOM manipulation**: useRef is commonly used to get a direct reference to a DOM element. This allows for direct manipulation of the DOM, such as managing focus, playing media, or integrating with third-party DOM libraries, in situations where React's declarative approach might be less suitable.
- **Storing mutable values**: It can be used to store any mutable value that needs to persist across renders but doesn't need to trigger a UI update. This could be timers, previous state values, or other non-reactive data.
