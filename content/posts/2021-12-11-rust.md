...
This pattern of deallocating resources at the end of an items lifetime is called *Resource Acquistion Is Initialisation* (RAII) in C++ circles.
...
As seen above, `s1` is moved to `s2`. Move semantics are great. A `String` uses both stack (a pointer to heap, length and capacity) and heap memory (the actual string bytes). `s2 = s1`, Rust copies the stack memory from `s1` into `s2`, then marks `s1` as invalid. This also highlights how Rust never copies heap memory automatically (i.e. never deep copies) and can always be trusted to be inexpensive in terms of runtime cost.
