...
Instead of just outputting straight binary signals, Mr Eater shows how to build a binary-to-decimal decoder circuit. In a nutshell the circuit will translate 8 bits of binary to its corresponding decimal value on series of 7 segment LED displays. AWESOME
...
But Mr Eater shows how to make your very own EEPROM programmer using an Arduino Nano. AWESOME++

We can then essentially write up some C code to store all the binary to 7-segment display mapping values into an EEPROM chip.
