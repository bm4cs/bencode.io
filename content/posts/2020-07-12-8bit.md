...
layout: post
draft: false
title: "8-bit computer from logic gates (Ben Eater)"
slug: "oidc"
date: "2020-07-12 20:08:55"
lastmod: "2022-05-22 19:26:35"
comments: false
categories:
  - cpu
tags:
...
- [Output register](#output-register)
  ...
  - [8-bit to 7-segment decoder](#8-bit-to-7-segment-decoder)
...
SSD (seven segment displays) are heaps on fun and often found in lots of devices such as microwaves. I purchased a bag of bright green (Gallium Phosphide Green) [Kingbright SC56-11GWA](/blob/datasheet-kingbright-7-segment-SC56-11GWA.pdf) common cathode 7-segment displays, which my computer will use to output upto 3 decimal digits.

I imagined this would involve quite a bit of mapping logic (e.g. `00000001` to turn on the 2 RHS vertical bars on the 7 segment display, `00000010` needs to turn on 5 segments the 3 horizontal bars and 2 vertical bars, and so on). Enter PROM's (programmable read-only memory). Using a PROM can write an arbitrary data bits to addresses, and it will remember what you set forever.
...
### 8-bit to 7-segment decoder

Now we're armed with a handy EEPROM burner, its time to program the ROM.

There many ways to load the mapping bitmap into the ROM. It all hinges on how you plan to wire the bus to the PROM, and the PROM to the 7-segment display. As long as everything is wired consistently fill your boots. One possible mapping scheme is as follows for a common cathode display. For common anode, flip the bits.
